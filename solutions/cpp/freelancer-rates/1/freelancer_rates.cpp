...
int monthly_rate(double hourly_rate, double discount) {
    ...
    // discount.
    double d_total = hourly_rate*8*22*(100-discount)/100;
    int truncated = (int)d_total;
    int total = truncated + (d_total>truncated);
    return total;
}
...
int days_in_budget(int budget, double hourly_rate, double discount) {
    ...
    int daily_rate = hourly_rate*8*(100-discount)/100;
